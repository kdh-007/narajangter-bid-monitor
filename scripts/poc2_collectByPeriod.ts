 ...
 *
 * 기존 scripts/poc2_collectByPeriod.ts 전체를 이 코드로 교체한다.
 * 기존 workflow / package.json / config/keywords.json을 그대로 사용한다.
 * PERIOD_START, PERIOD_END 환경변수로 기간 변경 가능.
 *
 * 추가 결과는 raw._poc2_enrichment에 저장한다.
 * 기존 DB 컬럼명 및 복합 UNIQUE 키를 그대로 사용한다.
 * 기존 기간 외 데이터는 삭제하지 않는다.
 *
 * 지역/면허 조회 중 하나라도 실패하면 해당 공고를 덮어쓰지 않는다.
 * Supabase 오류 발생 시 code/message/details/hint를 출력한다.
 * 42501 권한 오류는 첫 발생 시 전체 실행을 중단하고 종료코드 1을 설정한다.
 */
...
export function buildUrl(
  ...
  // Encoding 키와 Decoding 키 모두 한 번만 인코딩한다.
...
export async function fetchPage(
  ...
  for (let attempt = 1; attempt <= 5; attempt++) {
    ...
    try {
      ...
      if (code !== "00") {
        const safeCode = /^[A-Z0-9_]{1,60}$/i.test(code)
          ? code
          : "미확인";
      ...
      if (
        ...
        throw new ApiError("totalCount 오류");
      }
      ...
      const safeError =
        err instanceof ApiError
          ? err
          : new ApiError("연결·응답 수신 실패", true);

      if (!safeError.retryable || attempt === 5) {
        throw safeError;
      }

      console.warn(
        `  ${operation}: ${safeError.message}, 재시도 ${attempt}/5`
      );
    } finally {
      clearTimeout(timer);
    }
...
export async function fetchAll(
  ...
  for (let pageNo = 1; pageNo <= 10000; pageNo++) {
    ...
    if (fingerprint === previousPage) {
      throw new ApiError("동일 페이지 반복 수신");
    }
...
export async function main() {
  ...
  console.log(
    `수집 기간: ${start} 00:00 ~ ${end} 23:59 (한국 공고일 기준)`
  );
  ...
  for (const { label, operation } of OPERATIONS) {
    for (const { from, to } of chunks) {
      ...
      console.log(
        `[${label}] ${from}~${to}: ${items.length}건 수신`
      );
      ...
      for (const item of items) {
        ...
        const excluded = config.excludeKeywords.some(
          (kw: string) => title.includes(compact(kw))
        );

        const included = config.keywords.some(
          (kw: string) => title.includes(compact(kw))
        );
        ...
        try {
          ...
          const row = {
            ...
            raw: {
              ...
              _poc2_enrichment: {
                ...
                note:
                  "no_data는 API 조회 결과 없음이며 제한 없음이나 자격 충족을 뜻하지 않습니다.",
              },
            },
          };
          ...
          // 변경: Supabase가 반환한 상세 원인 출력.
          if (error) {
            console.error(
              `Supabase 저장 오류 [${label} ${no}-${ord}]:`,
              {
                code: error.code,
            ...
            // 변경: 권한 오류는 전체 작업을 즉시 중단.
            if (error.code === "42501") {
              failedNotices++;

              console.error(
                "Supabase 권한 오류(42501)로 수집을 중단합니다."
              );

              console.error(
                "위 message/details/hint를 확인해 테이블·시퀀스 권한 또는 RLS 정책을 점검하세요."
              );

              console.log(
                `중단 시점: 대상 ${matched}건 / 저장 성공 ${saved}건 / 공고 실패 ${failedNotices}건 / 목록 구간 실패 ${failedChunks}건`
              );
          ...
          const method =
            row.cntrct_mthd_candidate || "(값 없음)";

          distribution[method] =
            (distribution[method] ?? 0) + 1;

          console.log(
            `  저장 ${label} ${no}-${ord}: 지역 ${regions.length}건, 면허 ${licenses.length}건`
          );
        } catch (err) {
          failedNotices++;
  ...
  console.log(
    "계약방법 분포 (이번 실행 저장 성공 건):",
    distribution
  );

  console.log(
    `대상 ${matched}건 / 저장 성공 ${saved}건 / 공고 실패 ${failedNotices}건 / 목록 구간 실패 ${failedChunks}건`
  );

  console.log(
    `저장 성공 중 지역 API 결과 없음 ${regionEmpty}건 / 면허 API 결과 없음 ${licenseEmpty}건`
  );

  if (failedNotices || failedChunks) {
    throw new Error(
      `일부 수집·저장 실패. 저장 성공 ${saved}건이며, 로그 확인 후 재실행하세요.`
    );
  }

  console.log(
    "수집 완료. 기존 기간 외 데이터는 삭제하지 않았습니다."
  );
}
